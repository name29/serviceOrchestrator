...
class InfrastructureOrchstrator(object):
    ...
    def getTemplate(self, vnf_template_location):
        resp = requests.get(self.get_template % (vnf_template_location), headers=self.headers,
    ...
    def put(self, nffg):
        resp = requests.put(self.put_url, data=nffg.getJSON(domain=True), headers=self.headers,
                            timeout=int(self.timeout))
        resp.raise_for_status()
